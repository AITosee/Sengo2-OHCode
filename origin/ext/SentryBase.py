...
class SentryUartMethod:
    ...
    def Set(self, reg_address, value):
        ...
        while True:
            ...
            if err == SENTRY_PROTOC_OK:
                if data[0] == SENTRY_PROTOC_OK or data[1] == SENTRY_PROTOC_COMMADN_GET or data[2] == reg_address:
                    return SENTRY_OK
                else:
                    return data[0]
    ...
    def Get(self, reg_address):
        ...
        while True:
            err, data = self.__pkg_transmit(write_data)
            if err == SENTRY_PROTOC_OK:
                if data[0] == SENTRY_PROTOC_OK or data[1] == SENTRY_PROTOC_COMMADN_GET:
...
class SentryBase:
    ...
    def __init__(self,device_id, address=0x60, log_level=LOG_ERROR, kVisionQrCode = 9, VisionMaxType = 12):
        ...
        self.__img_h = 0
        self.__debug = None
        self.kVisionQrCode = kVisionQrCode
        self.kVisionMaxType = VisionMaxType
        self.__vision_states = [None]*VisionMaxType

        self.SetDebug(log_level)

    def __repr__(self):
        return 'SentryBase(device_id=%s, address=0x%02x, log_level=%s, kVisionQrCode=%s, VisionMaxType=%s)'%(self.__device_id, self.__address, self.__debug._level_str(__level__), self.kVisionQrCode, self.kVisionMaxType)
    ...
    def GetVisionState(self, vision_type):
        if vision_type >= self.kVisionMaxType:
            return 0
    ...
    def GetQrCodeValue(self):
        vision_state = self.__vision_states[self.kVisionQrCode-1]
